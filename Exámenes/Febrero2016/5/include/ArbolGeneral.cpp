...
template<class Tbase>
void ArbolGeneral<Tbase>::inorden(const ArbolGeneral<Tbase>::Nodo n)
{
    if (n != 0)
    {
        inorden(hijomasizquierda(n));
        cout << etiqueta(n) << ' ';
        inorden(hermanoderecha(n));
    }
}

template<class Tbase>
void ArbolGeneral<Tbase>::preorden(const ArbolGeneral<Tbase>::Nodo n)
{
    if (n != 0)
    {
        cout << etiqueta(n) << ' ';
        preorden(hijomasizquierda(n));
        preorden(hermanoderecha(n));
    }
}
