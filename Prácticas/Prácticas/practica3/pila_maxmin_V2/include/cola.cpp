...
template <class T>
T & Cola<T>::operator[](unsigned indice){
    Celda * aux = primera ;                     // Se crea un nuevo nodo con la cabezera actual

    for(unsigned i = -1 ; i < indice-1 ; i++)   // Se recorre la cola hasta llegar al índice
        aux = aux->siguiente ;

    return aux->elemento;                       // Se devuelve el elemento del índice
}
